...
class EleveCardWidget extends StatefulWidget {
  ...
  EleveCardWidget({super.key, required this.eleve, this.presentationMode});
...
class _EleveCardWidgetState extends State<EleveCardWidget> {
  Color changeColors() {
    return widget.eleve.id == EleveGestionFormGlobal.cp
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: () {
        ...
        if (EleveGestionFormGlobal.cp == widget.eleve.id &&
            widget.eleve.id != "") {
                  ...
                  onPressed: () {
                    showDialog(
                      ...
                      builder: (context) => AlertDialog(
                        title: Text(
                            'Êtes - vous sûre de vouloir supprimer ${widget.eleve.id}?'),
                        content: Row(
                              ...
                              TextButton(
                                  onPressed: () {
                                    Provider.of<StudentProvider>(context,
                                            listen: false)
                                        .removeEleveAndRef(widget.eleve);
                                    ScaffoldMessenger.of(context).showSnackBar(
                                        const SnackBar(
                                            content: Text(
                                                'L\'élève à était supprimé avvec succés')));
          ...
          Padding(
            ...
            child: CachedNetworkImage(
              ...
              placeholder: (context, url) =>
                  Image.asset('assets/img/placeholerImage.png'),
              width: 100,
            ),
          ),
